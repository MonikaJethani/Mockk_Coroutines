...
class GetRepositoriesUseCaseTest {
    ...
    fun testExecute_Positive() = runBlocking{
        coEvery { githubRepository.fetchRepositories("MonikaJethani") } returns listOf(ApiRepository("a", "aa"), ApiRepository("b", "bb"))

        val list = getRepositoriesUseCase.execute("MonikaJethani")
    ...
    fun testExecute_Negative() = runBlocking {
        coEvery { githubRepository.fetchRepositories("MonikaJethani") } throws IllegalStateException("err")
        try {
            val list = getRepositoriesUseCase.execute("MonikaJethani")
        } catch (e: Exception) {
